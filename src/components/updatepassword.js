...
import { resetPassword, clearAuthError, logout, updatepassword } from '../actions/userActions';


const Updatepassword = () => {
  const [oldPassword, setoldPassword] = useState('');
  const [newPassword, setnewPassword] = useState('');
  const { loading, error, isAuthenticated,user,message} = useSelector(state => state.authState);
          ...
          <div className='rbox'>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <label>
                ...
                />
                {shownewPassword ? (
                  <VisibilityOffIcon className="eye2" onClick={handleTogglenewPasswordVisibility} />
                ) : (
                  <VisibilityIcon className="eye2" onClick={handleTogglenewPasswordVisibility} />
                )}
              </label>

              <br />
              <button className='rbutt' type="submit">Update</button>
              <div>
                
                {user ? (user.role[0] === 'director' ?  <Link to='/Directorhome'><p className='link1'>Back to Home</p></Link> :  <Link to='/Writerhome'><p className='link1'>Back to Home</p></Link>) : (  <Link to='/'><p className='link1'>Back to Home</p></Link>)}
              </div>
            </form>
            <div>
            </div>
