  ...
  const handleDelete = async (scriptId) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        toast.success('Script deleted successfully!', { position: 'top-right' });

        // After successful deletion, you may want to refresh the script list
        dispatch(getAllScripts(user._id));
      } else {
        // Show an error toast message
        toast.error('Failed to delete script. Please try again.', { position: 'top-right' });
      }
    } catch (error) {
      console.error('Error deleting script:', error);
      // Show an error toast message
      toast.error('An error occurred while deleting the script.', { position: 'top-right' });
    }
  };
      ...
      <Container style={{ margin: '0' }}>
        <div>
          <p className='paraf' style={{ marginLeft: '15%', color: "gray" }}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed convallis faucibus est, vel tincidunt<br />
        ...
        <div className='textfield-container'>
          ...
          />
          <Button variant="contained" style={{ backgroundColor: '#14ABE4', padding: "15px 30px" }} onClick={clearSearch}>Clear</Button>
        </div>
      </Container>
              ...
              <div key={_id}>
                <div className='buttonconatiner'>
                  <div className='twobuttons-mobile' style={{ float: "right" }}>
                    ...
                    <Link to={`/updatescript/${_id}`} style={{ textDecoration: 'none' }}>
                      <Button variant="outlined" style={{ color: 'black', marginRight: "10px" }}>Edit</Button>
                    </Link>
                    <Button variant="contained" style={{ color: 'white' }} onClick={() => handleDelete(_id)}>Delete</Button>
